...
def generate_release_notes(output_path, version, bundles, previous_manifest_path):
    """Generate RELEASE_NOTES.md with changelog and bundle info."""
    ...
    if previous_manifest_path and os.path.exists(previous_manifest_path):
        with open(previous_manifest_path) as f:
            if previous_manifest_path.endswith('.json'):
                prev_manifest = json.load(f)
            else:
                prev_manifest = yaml.safe_load(f)
            previous_version = prev_manifest.get("version", "N/A")
            previous_bundles = prev_manifest.get("bundles", {})
...
| Repository | Type | Size | Previous Tag |
|------------|------|------|--------------|
"""

    for repo_name, info in bundles.items():
        size_kb = info['size'] / 1024
        size_str = f"{size_kb:.1f} KB" if size_kb < 1024 else f"{size_kb/1024:.1f} MB"
        from_tag = info.get('from_tag', 'N/A')
        notes += f"| {repo_name} | {info['type']} | {size_str} | {from_tag} |\n"

    notes += "\n---\n\n## 📊 Bundle Size Comparison\n\n"

    if previous_bundles:
        notes += "| Repository | Current Size | Previous Size | Change |\n"
        notes += "|------------|--------------|---------------|--------|\n"

        for repo_name, info in bundles.items():
            current_size = info['size']
            prev_info = previous_bundles.get(repo_name, {})
            prev_size = prev_info.get('size', 0)

            current_kb = current_size / 1024
            prev_kb = prev_size / 1024

            current_str = f"{current_kb:.1f} KB" if current_kb < 1024 else f"{current_kb/1024:.1f} MB"
            prev_str = f"{prev_kb:.1f} KB" if prev_kb < 1024 else f"{prev_kb/1024:.1f} MB"

            if prev_size > 0:
                change_pct = ((current_size - prev_size) / prev_size) * 100
                change_str = f"{change_pct:+.1f}%"
            else:
                change_str = "N/A (first release)"

            notes += f"| {repo_name} | {current_str} | {prev_str} | {change_str} |\n"
    else:
        notes += "*This is the first release - no comparison available.*\n"
...
3. **Load Git Bundles**
   - Follow TOHAD_UNPACK_INSTRUCTIONS.md for detailed steps
   - Clone bundles to local repos
   - Push to Tohad GitLab/GitHub
   - Merge incremental updates (if applicable)

---

## ⚠️ Important Notes

### For Incremental Bundles:
- Ensure previous release ({previous_version}) is loaded before applying this update
- Incremental bundles only contain changes since the previous tag
- If previous release is missing, request a full bundle from Byon

### For Tohad-Specific Files:
- Local customizations (e.g., `docker/overlays/tohad/`) will be preserved
- Git merge automatically handles files that don't exist in Byon repo
...
**Bundle Type:** {bundles.get('mock-app', {}).get('type', 'N/A')}

**Changes:**
- See git log in individual bundles for detailed commit history
- This release packages changes from {previous_version} to {version}

---

**Package generated by:** invoke create-release-package
**For support:** Contact Byon team
""".replace("{version}", version).replace("{previous_version}", previous_version)
...
def create_release(c, version="v1.0.0", previous_manifest=None, split=False, base64=False):
    ...
    # Save manifest as YAML (matches M3 spec)
    ...
    generate_release_notes(release_notes_path, version, bundles, previous_manifest)
    print(f"📝 Release notes saved: {release_notes_path}")
